...
import { chairmanFull } from '../mock';
import { Quote } from 'lucide-react';

const ChairmansMessagePage = () => {
  return (
    <PageLayout>
      <PageBanner title="Chairman's Message" breadcrumbs={[{ label: "Chairman's Message" }]} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            <div className="lg:col-span-4 lg:sticky lg:top-28">
              <div className="relative">
                ...
                <img
                  src={chairmanFull.photo}
                  alt={chairmanFull.name}
            ...
            <div className="lg:col-span-8">
              ...
              <div className="space-y-5 text-zinc-700 leading-[1.9] text-[16px] md:text-[17px]">
                {chairmanFull.paragraphs.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
              </div>
              <div className="mt-8 pt-6 border-t border-zinc-200">
                <p className="text-zinc-900 font-bold text-lg">{chairmanFull.name}</p>
                <p className="text-[#d99a0e] text-sm tracking-wider">{chairmanFull.designation}</p>
              </div>
            </div>
          </div>
        </div>
